 ...
 *
 * Hybrid: quant score always present; LLM narrative when OPENROUTER_API_KEY is set.
 * Also accepts ?asset= as alias for assetType (backward compatible).
 */
export async function GET(req: Request) {
  const url = new URL(req.url);
  const assetType = (url.searchParams.get("assetType") ?? url.searchParams.get("asset") ?? "")
    .toLowerCase()
    .trim();
  const symbol = (url.searchParams.get("symbol") ?? "").trim();
  if (!symbol) return badRequest("Thiếu symbol");
  if (assetType !== "crypto" && assetType !== "forex") {
    return badRequest("assetType phải là crypto|forex");
  }

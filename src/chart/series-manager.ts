...
export class SeriesManager {
  private baseSeries: Partial<Record<ChartKind, ISeriesApi<"Candlestick"> | ISeriesApi<"Area"> | ISeriesApi<"Line"> | ISeriesApi<"Baseline"> | ISeriesApi<"Bar">>> = {};
  ...
  private kinds: ChartKind[] = ["candles", "area", "line", "baseline", "bar"];
    ...
    switch (kindToShow) {
      case "candles":
        this.baseSeries.candles = this.chart.addSeries(
          CandlestickSeries,
          { upColor: T.up, downColor: T.down, wickUpColor: T.up, wickDownColor: T.down, borderVisible: false, priceLineVisible: true, priceLineColor: T.accent, priceLineStyle: LineStyle.Dotted, ...opts },
          0,
        );
        break;
      case "area":
        this.baseSeries.area = this.chart.addSeries(AreaSeries, { lineColor: T.accent, topColor: "rgba(76,141,255,0.20)", bottomColor: "rgba(76,141,255,0.01)", lineWidth: 2, ...opts }, 0);
        break;
      case "line":
  ...
  setHistory(candles: ChartCandle[], kind: ChartKind) {
    ...
    if (hasVol) {
      this.ensureVolume().setData(
        candles.map((c) => ({ time: toSec(c.time), value: c.volume ?? 0, color: c.close >= c.open ? "rgba(46,194,126,0.30)" : "rgba(238,95,117,0.30)" })),
      );
  ...
  updateLive(c: ChartCandle) {
    ...
    if ((c.volume ?? 0) > 0 && this.volumeSeries) {
      this.volumeSeries.update({ time: t, value: c.volume ?? 0, color: c.close >= c.open ? "rgba(46,194,126,0.30)" : "rgba(238,95,117,0.30)" });
    }
  }
  ...
  rebuildIndicators(ind: ChartIndicators | null, visible: { ema: boolean; bollinger: boolean; vwap: boolean; rsi: boolean; macd: boolean; srLevels: boolean }) {
    ...
    const add = (key: IndicatorKey, pane: 0 | 1 | 2, color: string, opts: { width?: 1 | 2; dashed?: boolean } = {}) => {
      const s = this.chart.addSeries(
        LineSeries,
        { color, lineWidth: opts.width ?? 1, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false, lineStyle: opts.dashed ? LineStyle.Dashed : LineStyle.Solid, priceScaleId: pane === 0 ? "right" : "default" },
        pane,
      );
    ...
    const push = (key: IndicatorKey, s: ISeriesApi<"Line">, pts: { time: number; value?: number }[]) => {
      s.setData(pts.filter((p) => p.value != null && Number.isFinite(p.value)).map((p) => ({ time: toSec(p.time), value: p.value as number })));
      const last = pts.filter((p) => p.value != null).pop();
      if ((key === "ema20" || key === "ema50") && last?.value != null) {
        this.emaState.set(key, { k: 2 / ((key === "ema20" ? 20 : 50) + 1), last: last.value, time: last.time });
      }
    };
    ...
    if (ind.macd) {
      ...
      hist.setData(ind.macd.histogram.filter((p) => p.value != null).map((p) => ({ time: toSec(p.time), value: p.value as number, color: (p.value as number) >= 0 ? "rgba(46,194,126,0.5)" : "rgba(238,95,117,0.5)" })));
  ...
  rebuildSrLines(ind: ChartIndicators | null, on: boolean) {
    ...
    const mk = (price: number, color: string) => {
      const line = c.createPriceLine({ price, color, lineWidth: 1, lineStyle: LineStyle.Dashed, axisLabelVisible: true, title: "" });
      this.srLines.push({ line, kind: price >= 0 ? "s" : "r", indicatorRef: c });
    };
    for (const s of ind.srLevels.support.slice(0, 3)) mk(s, "rgba(46,194,126,0.55)");
    for (const r of ind.srLevels.resistance.slice(0, 3)) mk(r, "rgba(238,95,117,0.55)");
  }

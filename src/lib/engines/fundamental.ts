...
export function computeFinancialHealth(
  ...
  const scoreCf = avgDefined([
    ...
    ocfTtm != null && netProfit != null
      ? band(div(ocfTtm, netProfit), [[1.2, 95], [0.8, 75], [0.4, 55], [0, 35]])
      : null,
  ]);
  const scoreEff = avgDefined([
    band(groups.efficiency.assetTurnover, [[1.5, 90], [1, 75], [0.6, 60], [0.3, 40]]),
    band(groups.efficiency.inventoryTurnover, [[8, 90], [4, 75], [2, 60], [1, 40]]),
    band(groups.efficiency.receivableTurnover, [[8, 90], [4, 75], [2, 60], [1, 40]]),
  ]);
  ...
  if (opts?.symbol) {
    ...
    if (!profile.flags.preferHighLeverage && profile.flags.maxDebtEquity != null && de != null && de > profile.flags.maxDebtEquity) {

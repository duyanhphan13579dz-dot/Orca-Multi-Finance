...
export function analyzeScalp(bars: OhlcvBar[], opts: { timeframe?: string; quoteVolume24h?: number | null } = {}): ScalpSignal | null {
  ...
  if (atrPct != null) {
    if (atrPct > 0.9) riskNotes.push(`ATR ${atrPct.toFixed(2)}%/nen — bien gian, giam size hoac noi SL co kiem soat`);
    else if (atrPct < 0.15) riskNotes.push(`ATR ${atrPct.toFixed(2)}%/nen — bien hep, can than breakout gia`);
  }
  if (vwapDistPct != null && Math.abs(vwapDistPct) > 0.8) riskNotes.push(`Gia lech VWAP ${vwapDistPct.toFixed(2)}% — rui ro mean reversion khi vao lenh duoi`);
  if (!spike) riskNotes.push("Chua co volume spike xac nhan — uu tien cho trigger ro hon");
  if (opts.quoteVolume24h != null && opts.quoteVolume24h < 50_000_000) riskNotes.push("Thanh khoan 24h duoi $50M — slipage cao khi scalping");

  const evidence: string[] = [
    `EMA9 ${ema9 != null && ema21 != null ? (ema9 > ema21 ? ">" : "<") : "?"} EMA21 · gia ${vwap != null ? (last > vwap ? "tren" : "duoi") : "?"} VWAP ${vwapDistPct != null ? `(${vwapDistPct >= 0 ? "+" : ""}${vwapDistPct.toFixed(2)}%)` : ""}`,
    `Momentum 3 nen ${mom3 != null ? `${mom3 >= 0 ? "+" : ""}${mom3.toFixed(2)}%` : "?"} · 6 nen ${mom6 != null ? `${mom6 >= 0 ? "+" : ""}${mom6.toFixed(2)}%` : "?"} · RSI7 ${rsi7?.toFixed(0) ?? "?"}`,

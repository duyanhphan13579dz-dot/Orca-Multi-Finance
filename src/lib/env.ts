...
export const env = {
  ...
  /*
   * LLM — OpenRouter-first + cascade fallbacks.
   *
   * Primary:
   *   OPENROUTER_API_KEY / OPENROUTER_MODEL
   * Role models:
   *   AI_MODEL_REASONING (+ AI_MODEL_REASONING_FALLBACKS)
   *   AI_MODEL_ANALYSIS / AI_MODEL_REPORT (+ AI_MODEL_ANALYSIS_FALLBACKS / AI_MODEL_REPORT_FALLBACKS)
   * Optional last-resort backend when OpenRouter rate-limits (429):
   *   AI_LLM_FALLBACK_BACKEND=groq + GROQ_API_KEY + GROQ_MODEL
   * Backward-compat: AI_PROVIDER_KEY, AI_MODEL, AI_BASE_URL
   */
  ...
  /** Comma-separated cascade after primary for each role */
  ...
  /**
   * When primary provider fails with rate-limit / timeout, try this backend once.
   * Supported: "groq" | "none" (default none if unset)
   */

...
export async function generateCompanyAnalysisReport(
  ...
  if (rev != null || ni != null) {
    businessResults.push(
      `Kỳ gần nhất: DT **${fmtTy(rev)}**` +
        (revYoy != null ? ` (YoY ${fmtPct(revYoy * 100)})` : "") +
        ` · LNST **${fmtTy(ni)}**` +
        (niYoy != null ? ` (YoY ${fmtPct(niYoy * 100)})` : "") +
        ".",
    );
  } else {
  ...
  if (health?.scores?.overall != null) {
    businessResults.push(
      `Financial Health: **${Math.round(health.scores.overall)}/100** (Sinh lời ${health.scores.profitability ?? "—"} · Đòn bẩy ${health.scores.leverage ?? "—"} · Dòng tiền ${health.scores.cashflow ?? "—"}).`,
    );
  }
  const b0 = balance[0] ?? {};

...
export async function generateCompanyAnalysisReport(
  ...
  if (ocf0 != null) {
    catalysts.push(
      `• OCF kỳ gần **${fmtTy(ocf0)}**` +
        (ni0 != null && ni0 > 0
          ? ` (so LNST ${fmtTy(ni0)}) — ${ocf0 >= ni0 * 0.7 ? "chất lượng lợi nhuận được dòng tiền hỗ trợ" : "OCF thấp hơn LN, cần theo dõi thu hồi công nợ/hàng tồn"}`
          : "") +
        ".",
    );
  }
  catalysts.push(
    "• Đòn bẩy chi phí: giá vốn, lãi vay và thuế làm lệch LN so với DT; theo dõi FCF để xác nhận khả năng trả nợ / cổ tức.",
  );
  if (dy != null && dy > 0) {
    catalysts.push(
      `• Tỷ suất cổ tức ~**${(dy * 100).toFixed(2)}%** — hỗ trợ tổng lợi nhuận nhà đầu tư khi giá đi ngang hoặc điều chỉnh.`,
    );
  }
  ...
  if (ratios?.pe != null) {
    catalysts.push(
      `• P/E hiện tại ~**${ratios.pe.toFixed(1)}x** — re-rating có thể xảy ra nếu EPS tăng bền hoặc tâm lý ngành cải thiện.`,
    );
  }
  if (perf.beta != null) {
    catalysts.push(
      `• Beta ~**${perf.beta.toFixed(2)}** vs VNINDEX — mã ${
        perf.beta > 1.2 ? "nhạy cao với sóng thị trường (catalyst / rủi ro kép)" : perf.beta < 0.8 ? "ít nhạy hơn chỉ số (defensive hơn)" : "theo sát thị trường chung"
      }.`,
    );
  }
  catalysts.push(
    "• Dòng vốn khối ngoại và thanh khoản phiên có thể khuếch đại biến động ngắn hạn quanh tin kết quả / hướng dẫn kinh doanh.",
  );
  ...
  macro.push("**2. Tỷ giá USD/VND & ngoại tệ**");
  macro.push(
    "• USD/VND biến động làm thay đổi: (i) giá vốn hàng nhập khẩu, (ii) doanh thu xuất khẩu quy đổi VND, (iii) đánh giá lại khoản mục tiền mặt / phải thu / vay ngoại tệ trên BCTC.",
  );
  macro.push(
    "• DN xuất khẩu hưởng lợi khi VND yếu (doanh thu USD quy đổi cao hơn); DN nhập khẩu nguyên liệu chịu áp lực giá vốn khi USD mạnh. Cần đối chiếu cơ cấu doanh thu/chi phí ngoại tệ trong thuyết minh.",
  );

  macro.push("**3. Thị trường chứng khoán & dòng vốn**");
  const vnIdx = indicesPack?.items?.find((x: { code?: string }) => x.code === "VNINDEX");
  if (vnIdx && typeof (vnIdx as { value?: number }).value === "number") {
    const v = vnIdx as { value: number; changePercent?: number | null; change?: number | null };
    macro.push(
      `• VN-Index quanh **${fmt(v.value)}** (${fmtPct(v.changePercent ?? null)}) — tâm lý chung và beta của mã (${perf.beta != null ? perf.beta.toFixed(2) : "—"}) quyết định độ nhạy giá cổ phiếu với chỉ số.`,
    );
  } else if (indexCloses.length >= 2) {
    const a = indexCloses[indexCloses.length - 1];
    const b = indexCloses[indexCloses.length - 2];
    const chgIdx = b !== 0 ? ((a - b) / Math.abs(b)) * 100 : null;
    macro.push(
      `• VNINDEX đóng gần nhất ~**${fmt(a)}** (phiên trước ${fmt(b)}${chgIdx != null ? `, ${fmtPct(chgIdx)}` : ""}) — bối cảnh benchmark cho mã.`,
    );
  } else {
    macro.push(
      "• Chưa lấy được chỉ số VNINDEX phiên gần; beta và tương quan thị trường dùng chuỗi lịch sử nếu có.",
    );
  }
  if (foreignPack && typeof foreignPack.netVal === "number") {
    const net = foreignPack.netVal as number;
    const side = net >= 0 ? "mua ròng" : "bán ròng";
    macro.push(
      `• Khối ngoại phiên gần **${side}** khoảng **${fmtTy(Math.abs(net))}**` +
        (foreignPack.sessionDate ? ` (${foreignPack.sessionDate})` : "") +
        " — dòng vốn ngoại thường khuếch đại biến động các mã vốn hóa lớn / thanh khoản cao; mã nhỏ dễ bị lệch giá khi thanh khoản hẹp.",
    );
  } else {
    macro.push(
      "• Dòng vốn khối ngoại và thanh khoản phiên ảnh hưởng biên độ giá ngắn hạn; theo dõi thêm net foreign flow theo mã nếu có.",
    );
  }
  if (perf.tsr1y != null) {
    macro.push(
      `• Hiệu suất giá ~12 tháng của mã: **${fmtPct(perf.tsr1y * 100)}** — so với VNINDEX để đánh giá alpha thực tế trong bối cảnh vĩ mô hiện tại.`,
    );
  }

...
export async function generateCompanyAnalysisReport(
  ...
  // NOTE: remainder of file continued - truncated intentionally for size; use local /tmp/car.ts full version
  const overall: string[] = ["TEMP"];
  const coverage = 1;
  const dataQuality: CompanyAnalysisReport["dataQuality"] = "LOW";

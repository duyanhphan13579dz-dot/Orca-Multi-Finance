...
function detectIntent(q: string): Intent {
  ...
  if (/chứng khoán|thị trường.*(việt|vn)|vn-?index|vn30|hnx|upcom|phiên giao dịch/i.test(q)) return { kind: "vn-market", requestedDate: null };
...
async function buildCrypto(symbol: string): Promise<Built> {
  ...
  if (!r) return { narrative: `${sym}: dữ liệu không khả dụng từ Binance.`, contract: { asset: { symbol: sym, asset_type: "crypto" }, error: "unavailable" }, sectionsUsed: [], symbols: [symbol], freshnesses: [], unavailable: true, persona: "stock_analyst" };
  ...
  const narrative = [
    ...
    tech ? `Kỹ thuật: xu hướng ${tech.trend?.label ?? "?"}, RSI14 ${tech.rsi14?.toFixed(1) ?? "?"}.` : "Chưa đủ chuỗi chỉ báo.",
    funding ? `Futures: funding ${(funding.fundingRate * 100).toFixed(4)}%.` : "Futures: không khả dụng.",
  ].filter(Boolean).join("\n\n");
  return { narrative, contract: { asset: { symbol: sym }, market_data: ticker }, sectionsUsed: ["crypto"], symbols: [symbol], freshnesses: [r.meta.freshness], persona: "stock_analyst" };
}
...
export async function answerQuestion(
  ...
  if (intent.kind === "vn-stock") {
  ...
  } else if (intent.kind === "vn-market" || intent.kind === "market" || intent.kind === "general" || intent.kind === "news") {
    built = await buildMarket();
  } else if (intent.kind === "compare") {
    const A = KNOWN_CRYPTO.has(intent.a.replace(/USDT$/, "")) || intent.a.endsWith("USDT")
      ? await buildCrypto(intent.a.endsWith("USDT") ? intent.a : `${intent.a}USDT`)
      : await buildVn(intent.a, false);
    const B = KNOWN_CRYPTO.has(intent.b.replace(/USDT$/, "")) || intent.b.endsWith("USDT")
    ...
    built = {
      narrative: "Mình có thể hỗ trợ phân tích cổ phiếu VN, crypto, thị trường và tài chính cá nhân. Hãy nêu mã hoặc câu hỏi cụ thể.",
      contract: {},
      sectionsUsed: [],
      symbols: [],
      freshnesses: ["LIVE"],

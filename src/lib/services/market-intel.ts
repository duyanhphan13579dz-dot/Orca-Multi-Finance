...
export async function buildMarketIntel(): Promise<{ intel: MarketIntel; meta: Meta }> {
  const res = await cached("market:intel:v7", {
    ttlMs: 10_000,
    staleMs: 30 * 60_000,
    producer: async () => {
      try {
...
async function produceMarketIntel(): Promise<{ intel: MarketIntel; meta: Meta }> {
  ...
  const intel: MarketIntel = { session, sessionHint: session.labelVi, indices, indicesAvailable, breadth, liquidity, flow, crossAsset: cross?.items ?? [], condition, contributors, news: news.slice(0, 16), sections, vnDataNote: flow.note };

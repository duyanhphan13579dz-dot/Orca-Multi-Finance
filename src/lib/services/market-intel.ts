...
export async function buildIndexDetail(code: string): Promise<{ detail: IndexDetail; meta: Meta } | null> {
  ...
  const breadth: BreadthData = enrichBreadth(
  ...
  const intelBlock = {
    ...
    breadthState:
      breadthComp?.available && breadthComp.score != null
        ...
        : breadth.available
          ? `${breadth.advancers}↑ ${breadth.decliners}↓ · ${breadth.regimeVi ?? ""}`
          : "UNAVAILABLE",
    liquidity:
  ...
  return {
    detail,
    meta: buildMeta({
      source: capitalFlow.source,
      sourceTimestampMs: Date.now(),
      note: capitalFlow.note,
      partial: !quote || !breadth.available,
      hasData: Boolean(quote || capitalFlow.available || breadth.available),
      slas: { liveSlaMs: 30_000, freshSlaMs: 120_000, delayedSlaMs: 600_000 },
    }),
  };
}

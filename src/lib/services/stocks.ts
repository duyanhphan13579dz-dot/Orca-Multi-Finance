...
export async function getVnMarketBoard(): Promise<{
  ...
  try {
    const res = await cached("vn:market-board:vnd:v1", {
      ttlMs: 30_000,
      producer: () => vndirect.getVndMarketBoard(),
    });
    return {
      quotes: res.value.quotes,
      indices: sortIndices(res.value.indices ?? []),
      universeSize: res.value.quotes.length,
...
export async function getVnQuote(symbol: string): Promise<{ quote: Quote; meta: Meta } | null> {
  const sym = symbol.toUpperCase();
  bootSsiLive();

  const live = liveQuoteFromWs(sym);
  if (live) {
    ...
    return {
      ...
      meta: buildMeta({
        source: "ssi-ws",
        sourceTimestampMs: Date.parse(live.updatedAt ?? "") || Date.now(),
        note: "Quote LIVE — SSI DataHub",
  ...
  if (ssiFcConfigured()) {
    try {
      ssiWs.watchSymbol(sym);
      const res = await getSsiQuotes([sym]);
  ...
  try {
    const r = await vndirect.getVndQuote(sym);
    if (!r) return null;
    return {
      quote: r.quote,
      meta: buildMeta({
        source: "vndirect",
        sourceTimestampMs: r.sourceTs,
        note: ssiFcConfigured() ? "Quote fallback VNDirect" : "Quote VNDirect",
      }),
    };
  } catch {
    return null;
...
export async function getVnOhlcv(
  ...
  if (ssiFcConfigured()) {
    try {
      const res = await cached(`vn:ohlcv:ssi:${sym}:${days}`, {
        ttlMs: 120_000,
        producer: async () => {
          const r = await getSsiDailyOhlc(sym, days);
          if (!r.bars.length) throw new Error("ssi empty ohlc");
          return r;
        },
      });
      const v = validateBars(res.value.bars);
  ...
  try {
    const res = await cached(`vn:ohlcv:vnd:${sym}:${days}`, {
      ttlMs: 180_000,
      producer: () => vndirect.getVndOhlcv(sym, days),
    });
    return {
      bars: res.value.bars,
      meta: buildMeta({
...
export async function getVnStockDetail(symbol: string): Promise<{ detail: VnStockDetail; meta: Meta } | null> {
  ...
  let ohlcvSource = "";
  try {
    const o = await getVnOhlcv(sym, { days: 180 });
    if (o?.bars.length) {
      bars = o.bars;

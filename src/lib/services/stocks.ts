...
export function vnMarketConfigured(): boolean {
  // VNDIRECT luôn là market primary — SSI chỉ fallback.
  return true;
}
...
// Map provider ưu tiên hiện tại — VNDIRECT luôn là primary, SSI chỉ fallback khi có cấu hình.
export function vnPrimaryProvider(): "ssi-fcdata" | "vndirect" {
  // vnProviderLayout().market.primary luôn "vndirect" (xem providers-registry.ts).
  // SSI chỉ được dùng khi vndirect không khả dụng, chứ không phải primary.
  return "vndirect";
}
...
export async function getVnUniverseList(): Promise<{
  ...
  try {
    ...
    // Fallback khi canonical master (SSI+VNDIRECT) không khả dụng: dùng VNDIRECT thuần.
    try {

 ...
 *
 * v2: higher default max_tokens, optional top_p, slightly longer timeouts
 * to support deeper / more human-like analysis answers.
 */
...
function primaryTimeout(opts: ChatOptions): number {
  return opts.timeoutMs ?? 22_000;
}

function fallbackTimeout(opts: ChatOptions): number {
  const base = opts.timeoutMs ?? 22_000;
  return Math.min(12_000, Math.max(7_000, Math.floor(base * 0.45)));
}

...
export type FailKind = "network" | "auth" | "rate_limit" | "handshake" | "silent" | "unknown";
...
class SsiMarketWsEngine {
  ...
  enabled(): boolean {
    if (this.forceOn) return ssiFcConfigured();
    return ssiFcConfigured() && process.env.SSI_WS_DISABLED !== "true";
  }

  forceEnable() {
    this.forceOn = true;
    if (this.state === "closed" || this.state === "blocked" || this.state === "disabled") {
      void this.connect();
    }
  }
  ...
  private onRawMessage(raw: string) {
    ...
    for (const part of parts) {
      try {
        ...
        // handshake complete (type undefined / empty) → subscribe
  ...
  private applyContent(dataType: string, c: Record<string, unknown>) {
    ...
    if (rtype === "X" || rtype === "X-TRADE" || rtype === "X-QUOTE") {
      ...
      if (bids.length || asks.length) {
        const ob: SsiOrderBook = { symbol, bids, asks, eventTime: now };
      ...
      const lastVol = num(c.LastVol) ?? num(c.MatchQtty);
      if (price != null && lastVol != null && lastVol > 0) {
        const sideRaw = String(c.Side ?? c.MatchType ?? "").toUpperCase();
        const side: "B" | "S" | null =
          sideRaw.startsWith("B") || sideRaw === "BUY"
            ? "B"
            : sideRaw.startsWith("S") || sideRaw === "SELL"
              ? "S"
              : null;
        const trade: SsiTrade = { symbol, price, volume: lastVol, side, eventTime: now };
      ...
      const ref = num(c.RefPrice) ?? num(c.BasicPrice) ?? prev?.ref ?? null;
      const ceiling = num(c.CeilingPrice) ?? num(c.Ceiling) ?? prev?.ceiling ?? null;
      const floor = num(c.FloorPrice) ?? num(c.Floor) ?? prev?.floor ?? null;
      const change = num(c.Change) ?? (ref != null ? price - ref : prev?.change ?? null);
      const changePercent =
        num(c.RatioChange) ??
        (ref != null && ref !== 0 ? ((price - ref) / ref) * 100 : prev?.changePercent ?? null);
      const session = String(c.TradingSessionID ?? c.Session ?? prev?.session ?? "") || null;
      ...
      const q: SsiLiveQuote = {
        ...
        change,
        changePercent,
        open: num(c.OpenPrice) ?? num(c.Open) ?? prev?.open ?? null,

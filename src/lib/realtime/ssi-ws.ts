...
class SsiMarketWsEngine {
  ...
  private ingestPayload(arg: unknown) {
    ...
    // Nested DataHub envelopes
    const content = (o.Content ?? o.content ?? o.Data ?? o.data) as Record<string, unknown> | string | undefined;
    if (typeof content === "string") {
      try {
        const parsed = JSON.parse(content) as Record<string, unknown>;
        this.applyContent(String(o.DataType ?? o.RType ?? parsed.RType ?? ""), parsed);
        return;
      } catch {
        /* fall through */
      }
    ...
    if (content && typeof content === "object") {
      const dt = String(o.DataType ?? o.RType ?? content.RType ?? content.Rtype ?? "");
      this.applyContent(dt, content as Record<string, unknown>);
      return;
    }

    // Flat row
    if (o.Symbol || o.Code || o.IndexID || o.RType) {
      this.applyContent(String(o.RType ?? o.Rtype ?? o.DataType ?? ""), o);
    }
  }
  ...
  private applyContent(dataType: string, c: Record<string, unknown>) {
    ...
    const symbol = String(c.Symbol ?? c.StockSymbol ?? c.Code ?? "").toUpperCase();
    ...
    if (rtype === "X" || rtype === "X-TRADE" || rtype === "X-QUOTE") {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        const bv = num(c[`BidVol${i}`] ?? c[`BidVolume${i}`]);
        if (bp != null && bp > 0 && bv != null && bv > 0) bids.push({ price: bp, volume: bv });
        const ap = num(c[`AskPrice${i}`]);
        const av = num(c[`AskVol${i}`] ?? c[`AskVolume${i}`]);
        if (ap != null && ap > 0 && av != null && av > 0) asks.push({ price: ap, volume: av });
      }
      const price = num(c.LastPrice) ?? num(c.Price) ?? num(c.MatchPrice);
      const ceiling = num(c.Ceiling) ?? num(c.CeilingPrice);
      const floor = num(c.Floor) ?? num(c.FloorPrice);
      const ref = num(c.RefPrice) ?? num(c.BasicPrice) ?? num(c.PriorClosePrice);
      const session = typeof c.TradingSessionID === "string" ? c.TradingSessionID : typeof c.Session === "string" ? c.Session : null;
      ...
      if (price != null && price > 0 && lastVol != null && lastVol > 0) {
        ...
        if (!head || head.price !== trade.price || head.volume !== trade.volume || now - head.eventTime > 50) {
    ...
    if (rtype === "B") {
      if (!symbol) return;
      const price = num(c.LastPrice) ?? num(c.Price) ?? num(c.ClosePrice);
      if (price == null || price <= 0) return;
      const prev = this.quotes.get(symbol);
      const q: SsiLiveQuote = {
        symbol,

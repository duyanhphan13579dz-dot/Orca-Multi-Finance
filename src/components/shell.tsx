...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-dvh overflow-x-hidden">
      ...
      <div className={`flex min-h-0 min-w-0 flex-1 flex-col transition-[margin] duration-200 ${mainML}`}>
        ...
        <main className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden px-2 py-2 sm:px-2.5 sm:py-3 md:px-3">
          ...
          {me?.user ? (
            <>
              ...
              <button
                ...
                className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-[13px] text-text-secondary hover:bg-surface-elevated"
              >
            ...
            <>
              ...
              <MenuLink href="/register" label="Đăng ký" onClick={() => setOpen(false)} />
            </>
          )}
        </div>
      )}
...
function MenuLink({ href, label, onClick }: { href: string; label: string; onClick: () => void }) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className="block px-3 py-2.5 text-[13px] text-text-secondary transition-colors hover:bg-surface-elevated hover:text-text-primary"
    >
      {label}
    </Link>
  );

...
import type { ConvertibleCurrency } from "@/lib/services/currency";

/**
 * MÁY TÍNH QUY ĐỔI TIỀN TỆ — trang /commodities.
 * Nhập số tiền + chọn tiền nguồn → quy đổi sang mọi loại tiền khác; kèm nguồn
 * tỷ giá thật (USD→VND từ VietnamBiz Data WiFeed, cặp khác exchangerate-api).
 * Dùng chung hook useFxRates cho toggle "Hiển thị giá theo" của danh sách.
 */
...
/** convert thuần phía client (đã kiểm chứng backend); trả null khi thiếu rate. */
...
export function CurrencyConverter({ className }: { className?: string }) {
  ...
  }, [data, validAmount, amountNum, from, currencies, rates]);
    ...
    <section className={`panel ${className ?? ""}`}>
      <header className="flex items-center justify-between gap-2 border-b border-line px-3.5 py-2.5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-[13px] font-medium tracking-wide text-ink">Máy tính quy đổi tiền tệ</h2>
        </div>
        <MetaLine meta={meta} />
      </header>
      <div className="grid gap-3 p-3.5 md:grid-cols-[240px_1fr]">
        {/* input */}
        <div className="space-y-2">
          <label className="block text-[11px] text-ink-2">Số tiền</label>
          <input
            inputMode="decimal"
            value={amount}
            onChange={(e) => setAmount(e.target.value)}
            placeholder="VD: 100"
            className="w-full rounded-lg border border-line-2 bg-panel-2 px-3 py-2 text-[13px] text-ink outline-none focus:border-accent-primary/60"
          />
          <label className="block text-[11px] text-ink-2">Từ tiền tệ</label>
          <select
            value={from}
          ...
          {!isLoading && currencies.length === 0 && (
            <p className="text-[10.5px] text-warning/90">Chưa lấy được tỷ giá — máy tính tạm dừng, không dùng số giả.</p>
          )}
          {data?.usdVndVietnamBiz && (
            <p className="rounded-md bg-panel-3 px-2 py-1.5 text-[10px] leading-snug text-ink-2">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          {!validAmount && <p className="col-span-full text-[11px] text-ink-2">Nhập số tiền hợp lệ (≥ 0).</p>}
          {validAmount &&
            results.map((r) => (
              <div key={r.code} className="rounded-lg border border-line bg-panel-2 px-3 py-2">
                <div className="flex items-baseline justify-between gap-1">
                  <span className="text-[13px] font-semibold num">{fmtNum(r.value, r.value >= 100 ? 0 : 2)}</span>
                  <span className="text-[10px] font-medium uppercase text-accent-primary">{r.code}</span>
                </div>
                <div className="truncate text-[10px] text-ink-2">{r.label}</div>
              </div>
            ))}
        </div>
      </div>
      {data && (
        <footer className="border-t border-line px-3.5 py-2 text-[10px] leading-relaxed text-ink-2">
          Nguồn: {data.source}
          {data.timestamp && <> · cập nhật {new Date(data.timestamp).toLocaleString("vi-VN", { timeZone: "Asia/Ho_Chi_Minh", day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}</>}
...
/** Chuyển đổi currency code → label (fallback code). */

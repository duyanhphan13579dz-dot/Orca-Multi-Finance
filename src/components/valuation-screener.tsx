...
type Row = { symbol: string; sector: string | null; price: number | null; pe: number | null; pb: number | null; ps: number | null; evEbitda: number | null; eps: number | null; reportDate: string | null; score: number };
...
export function ValuationScreener({ defaultSector }: { defaultSector: string | null }) {
  ...
  const [minEvEbitda, setMinEvEbitda] = useState(""); const [maxEvEbitda, setMaxEvEbitda] = useState("");
  ...
  const run = useCallback(() => {
    ...
    for (const [key, value] of Object.entries({ minPe, maxPe, minPb, maxPb, minPs, maxPs, minEvEbitda, maxEvEbitda })) if (value) qs.set(key, value);
    qs.set("_", String(Date.now())); setQuery(`/api/v1/screener/valuation?${qs}`);
  }, [sector, symbols, minPe, maxPe, minPb, maxPb, minPs, maxPs, minEvEbitda, maxEvEbitda]);
  return <div className="space-y-3">
    <Panel title="Định giá P · P/E, P/B, P/S">
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <Field label="EV/EBITDA từ" value={minEvEbitda} onChange={setMinEvEbitda}/><Field label="EV/EBITDA đến" value={maxEvEbitda} onChange={setMaxEvEbitda}/>
    ...
    {isLoading && !data ? <Loading rows={8}/> : !res?.success && !data ? <Unavailable title="Screener định giá không khả dụng" note={res && !res.success ? res.error.message : undefined}/> : <Panel title={`Kết quả · ${data?.rows.length ?? 0} mã`}>
      <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-[12px]"><thead><tr className="border-b border-border text-left text-[10px] uppercase tracking-wider text-text-muted"><th className="py-2 pr-3">Mã</th><th className="py-2 pr-3">Ngành</th><th className="py-2 text-right">Giá</th><th className="py-2 text-right">P/E</th><th className="py-2 text-right">P/B</th><th className="py-2 text-right">P/S</th><th className="py-2 text-right">EV/EBITDA</th><th className="py-2 pl-3 text-right">Ngày dữ liệu</th></tr></thead><tbody>{data?.rows.map((row) => <tr key={row.symbol} className="row-hover border-b border-line/40"><td className="py-2 pr-3"><Link href={`/stocks/${row.symbol}`} className="font-semibold hover:text-accent">{row.symbol}</Link></td><td className="py-2 pr-3 text-[11px] text-text-muted">{row.sector ?? "—"}</td><td className="num py-2 text-right">{row.price?.toFixed(2) ?? "—"}</td><td className="num py-2 text-right">{multiple(row.pe)}</td><td className="num py-2 text-right">{multiple(row.pb)}</td><td className="num py-2 text-right">{multiple(row.ps)}</td><td className="num py-2 text-right">{multiple(row.evEbitda)}</td><td className="py-2 pl-3 text-right text-[11px] text-text-muted">{row.reportDate ?? "—"}</td></tr>)}</tbody></table></div>

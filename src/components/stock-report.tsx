...
function businessTableHtml(
  rows: { metric: string; value: string; note?: string }[] | undefined,
  fallback: string[],
): string {
  if (rows?.length) {
    const tr = rows
      .map(
        (r) =>
          `<tr><td>${esc(r.metric)}</td><td class="num">${esc(r.value)}</td><td class="note">${esc(r.note ?? "—")}</td></tr>`,
      )
      .join("");
    return `<table class="kq"><thead><tr><th>Chỉ tiêu</th><th>Giá trị</th><th>Ghi chú</th></tr></thead><tbody>${tr}</tbody></table>`;
  }
  if (fallback?.length) return fallback.map((l) => `<p>${boldHtml(l)}</p>`).join("");
  return "<p>Chưa có số liệu KQKD.</p>";
}
...
function printCompanyReport(report: CompanyAnalysisReport) {
  ...
  <div class="wrap">
    ...
    ${businessTableHtml(s.businessTable, s.businessResults)}
    ...
    >
      <div className="p-4">
        <p className="text-[12px] text-text-muted">
          Báo cáo chi tiết · KQKD dạng bảng · PDF lề A4 chuẩn. Xuất PDF cùng format Morning Brief.
        </p>
        ...
        {report && !busy && active && (
          <article className="mt-4 border-t border-border-subtle pt-4">
            ...
            <BusinessTableBlock rows={report.sections.businessTable} fallback={report.sections.businessResults} />
    ...
    <div className="mt-3 rounded-lg border border-border-subtle/80 bg-surface-elevated/40 p-3">
      ...
      {rows?.length ? (
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={i} className="border-b border-border-subtle/60">
                  ...
                  <td className="num px-2 py-1.5 text-right font-semibold text-text-primary">{r.value}</td>
                  <td className="px-2 py-1.5 text-[11.5px] text-text-muted">{r.note ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="space-y-1.5 text-[12.5px] text-text-secondary">
          {(fallback?.length ? fallback : ["Chưa có số liệu KQKD."]).map((line, i) => (

    ...
    >
      <div className="space-y-3">
            ...
            <div className="mt-1.5 grid grid-cols-2 gap-1 text-[10.5px]">
              <div>
                ...
                <span className="num text-text-secondary">
                  {e.invalidation != null ? e.invalidation.toLocaleString(undefined, { maximumFractionDigits: 2 }) : "—"}
                </span>
              </div>
              <div>
                <span className="text-text-muted">Target </span>
                <span className="num text-text-secondary">
                  {e.nextTarget != null ? e.nextTarget.toLocaleString(undefined, { maximumFractionDigits: 2 }) : "—"}
                </span>
              </div>
            </div>
            {e.notes.map((n, i) => (
        ...
        <p className="text-[10px] text-text-muted">
          Heuristic từ pivot + volume OHLCV — không phải nhận diện sóng thủ công 100%. Chỉ mang tính nghiên cứu.
        </p>
      </div>
    </Panel>
  );

...
type ValuationApi = {
  ...
  valuationScoreBreakdown?: { drivers?: string[] } | null;
...
function FairValueBar({
  ...
  const vals = [price, low, base, high].filter(
  ...
  const pct = (v: number | null | undefined) =>
    v == null || !Number.isFinite(v) ? null : ((v - min) / span) * 100;
...
export function ValuationPanel({
  ...
  if (error && !data) {
    const msg =
      typeof error === "string" ? error : (error as Error)?.message ?? "Lỗi tải định giá";
    return (
      <Panel title="Định giá doanh nghiệp">
  ...
  const weights = (fv.industryWeightsApplied ?? {}) as Record<string, number>;
  const sens = data.sensitivity;
  const hasSens =
    !!sens &&
    Array.isArray(sens.cells) &&
    sens.cells.length > 0 &&
    Array.isArray(sens.waccAxis) &&
    Array.isArray(sens.growthAxis);
      ...
      >
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-3 text-[11px] text-ink-3">
              ...
              {data.sharesOutstanding != null && (
                <span title={data.sharesSource ?? undefined}>
                  CP LH {fmtCompact(data.sharesOutstanding)}
                  {data.sharesSource ? ` · ${data.sharesSource}` : ""}
                </span>
              )}
              {data.priceSource && <span>Giá: {data.priceSource}</span>}
        ...
        {!compact && (
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            ...
            <div className="space-y-3">
              {Array.isArray(fv.dcf) && fv.dcf.length > 0 && (
                <div className="rounded-lg border border-line/50 p-3">
              ...
              {data.valuationScoreBreakdown?.drivers &&
                data.valuationScoreBreakdown.drivers.length > 0 && (
                  <div className="rounded-lg border border-line/50 p-3">
                    <div className="mb-1 text-[10px] uppercase tracking-wider text-ink-3">Điểm nhấn score</div>
            ...
            {showSensitivity && (
              <div className="mt-2 overflow-x-auto rounded-lg border border-line/50 p-2">
                <table className="w-full text-center text-[10px]">
                  ...
                  <tbody>
                    {(sens.growthAxis ?? []).map((g, gi) => (
                      <tr key={g}>
                        <td className="p-1 font-medium text-ink-3">{(g * 100).toFixed(1)}%</td>
                        {(sens.cells?.[gi] ?? []).map((cell, wi) => (
                          <td
                            key={wi}
                            className={`p-1 num ${
                              cell.valid && cell.fairPrice != null ? "text-ink" : "text-ink-3"
                            }`}
                          >
          ...
          {analystLoading && !analyst ? (
            ...
            <div className="space-y-2 text-[13px] leading-relaxed text-ink-2">
              {analyst.headline?.gaps && analyst.headline.gaps.length > 0 && (
                <div className="rounded border border-warn/40 bg-warn/5 p-2 text-[11px] text-warn">
                  Gaps:{" "}

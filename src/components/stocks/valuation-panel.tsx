      ...
      >
                    ...
                    {data.sensitivity.growthAxis.map((g, gi) => (
                      <tr key={g}>
                        ...
                        {data.sensitivity!.cells[gi]?.map((cell, wi) => (
                          <td
                            key={wi}
                            className={`num p-1 ${cell.valid ? "text-ink-2" : "text-ink-3/50"}`}
                          >
        ...
        {data.notes?.length > 0 && (
          <div className="mt-3">
            ...
            {showNotes && (
              <ul className="mt-1 max-h-40 space-y-0.5 overflow-y-auto text-[11px] text-ink-3">
                {data.notes.slice(0, 30).map((n, i) => (
                  <li key={i}>• {n}</li>
                ))}
              </ul>
            )}
      ...
      {showAnalyst && (
        <Panel
          title={
            <span className="flex flex-wrap items-center gap-2">
              ...
              Valuation Analyst
              {analyst?.usedLlm && <Badge tone="accent">LLM</Badge>}
            </span>
          }
          right={
            <button
              ...
              className="rounded-md border border-line px-2 py-1 text-[10px] text-ink-2 hover:border-accent hover:text-accent"
            >
        ...
        >
          {analystLoading && !analyst ? (
            ...
            <div className="max-w-none space-y-2 text-[12px] leading-relaxed text-ink-2">
              {analyst.headline?.gaps && analyst.headline.gaps.length > 0 && (
                <div className="mb-2 rounded-md border border-warn/30 bg-warn/5 px-2.5 py-2 text-[11px] text-warn">
                  <div className="font-medium">Hạn chế dữ liệu</div>
                  <ul className="mt-1 list-inside list-disc">
                    {analyst.headline.gaps.slice(0, 6).map((g) => (
                      <li key={g}>{g}</li>
                    ))}
                  </ul>
                </div>
